...
cp ../Electrum-Lite-1.9.7.tar.gz electrum.tar.gz
tar xvf electrum.tar.gz
mv Electrum-Lite-1.9.7 electrum-lite
rm -rf $WINEPREFIX/drive_c/electrum-lite
cp electrum-lite/LICENCE .
mv electrum-lite $WINEPREFIX/drive_c
...
$PYTHON "C:/pyinstaller/pyinstaller.py" --noconfirm --ascii -w --onefile "C:/electrum-lite/electrum-lite"
...
cd dist
mv electrum-lite.exe $NAME_ROOT.exe
#mv electrum $NAME_ROOT
#mv electrum-lite-setup.exe $NAME_ROOT-setup.exe
