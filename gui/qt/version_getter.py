...
from electrum_lite import ELECTRUM_VERSION, print_error
...
class VersionGetter(threading.Thread):
    ...
    def run(self):
        try:
            con = httplib.HTTPConnection('electrum-lite.org', 80, timeout=5)
...
class UpdateLabel(QLabel):
    ...
    def open_website(self):
        webbrowser.open("http://electrum-lite.org/download.html")
        self.dialog.done(0)

    def mouseReleaseEvent(self, event):
        dialog = QDialog(self)
        dialog.setWindowTitle(_('Electrum Lite update'))
        dialog.setModal(1)

        main_layout = QGridLayout()
        main_layout.addWidget(QLabel(_("A new version of Electrum Lite is available:")+" " + self.latest_version), 0,0,1,3)
